 ...
 */
export const estimateComplexity = (totalOps, n) => {
  if (n < 2) return { best: 'O(1)', average: 'O(1)', worst: 'O(1)' }

  const log2n  = Math.log2(n)
  const nlogn  = n * log2n
  const nSq    = n * n

  // Allow 20 % headroom for constants
  if (totalOps <= n * 1.5)         return { best: 'O(1)',       average: 'O(n)',        worst: 'O(n)'      }
  if (totalOps <= nlogn * 2.5)     return { best: 'O(n log n)', average: 'O(n log n)',  worst: 'O(n log n)'}
  if (totalOps <= nSq  * 1.5)      return { best: 'O(n)',       average: 'O(n²)',       worst: 'O(n²)'     }
...
export const runCustomAlgorithm = (inputArr, code) => {
  ...
  try {
    ...
    return {
      ...
      error:       err.message,
      complexity:  estimateComplexity(comparisons + swaps, inputArr.length)
    }
  }
